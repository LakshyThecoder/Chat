...
import { registerFlightDeskExtraTools } from "@/components/flight-desk/register-flight-desk-tools";
import {
...
function bindNow(onStatus: (status: TheaterWebMcpStatus) => void): boolean {
  try {
    ...
    const extra = registerFlightDeskExtraTools(context);
    void fetch("/api/demo/theater/session", { method: "GET" }).catch(() => undefined);
    const discovered = discoverRegisteredToolNames(context);
    const catalogCount = names.length + extra.length;
    const status = {
      ready: true,
      reason: `WebMCP online · ${discovered.length > 0 ? discovered.length : catalogCount} tools on this desk.`,
      tools: discovered.length > 0 ? discovered : [...names, ...extra],
    };
    onStatus(status);
    publishStatus(status);
    return true;
...
export function FlightDeskWebMcp({
  onStatus,
}: {
